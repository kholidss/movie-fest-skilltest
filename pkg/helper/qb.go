...
func StructQueryWhereLikeMysql(i interface{}, hideDeleted bool, tag string) (q string, vals []interface{}, limit, page uint64, err error) {

	var cols, colsLike []string
	var valsLike []interface{}
	var startDate, endDate, sortOrder, groupBy string
	...
	for k, x := range data {
		...
		if strings.HasPrefix(k, "like_") {
			valsLike = append(valsLike, "%"+cast.ToString(x)+"%")
			colsLike = append(colsLike, strings.TrimPrefix(k, "like_"))
			continue
		}
	...
	if len(colsLike) > 0 {
		q = fmt.Sprintf(`%s AND %s`, q, util.StringJoin(colsLike, " LIKE ? AND ", " LIKE ?"))
		vals = append(vals, valsLike...)
	}
